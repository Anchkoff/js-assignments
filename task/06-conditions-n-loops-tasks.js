...
function isTriangle(a,b,c) {
    var sorted_sides = [a, b, c].sort((a, b) => b-a);
...
function getCommonDirectoryPath(pathes) {
    var common_path = pathes[0];
    pathes.forEach((path) => {
        var i = 0;
        var min_length = Math.min(common_path.length, path.length)
...
function evaluateTicTacToePosition(position) {
    ...
    for (var i of winning_lines) {
        var first_cell = position[i[0][0]][i[0][1]];
        var second_cell = position[i[1][0]][i[1][1]];
        var third_cell = position[i[2][0]][i[2][1]]
        if (first_cell && first_cell == second_cell && first_cell == third_cell) {
            return first_cell;
        }
    }
    return undefined;
